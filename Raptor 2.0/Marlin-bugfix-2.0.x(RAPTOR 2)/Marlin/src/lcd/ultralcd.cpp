...
uint16_t max_display_update_time = 0;
  ...
  #ifdef SPINDLE_LASER_ENABLE_PIN
    void update_laser_status();
...
void lcd_quick_feedback(const bool clear_buttons) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void update_laser_status() {
      ...
      if (laser_on)
        enqueue_and_echo_commands_P(PSTR("M3"));
      else
        enqueue_and_echo_commands_P(PSTR("M5"));
    }
  #endif
  ...
  void lcd_main_menu() {
    ...
    MENU_ITEM_EDIT_CALLBACK(bool, MSG_LASER_ON_OFF , (bool*)&laser_on, update_laser_status);
